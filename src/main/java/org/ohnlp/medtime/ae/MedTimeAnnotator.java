...
public class MedTimeAnnotator extends JCasAnnotator_ImplBase {
    ...
    private String PARAM_RESOURCE_DIR = "MedTime_Resource_Dir";
...
//need to do.. work if we have the proper sectionization
...
//					"to discharge date to avoid algorithm error");
    ...
    public boolean specifyAmbiguousValues(JCas jcas) {
        ...
        int dctWeekday = 0;
        int dctWeek = 0;
        //---Specifically for the doc 1st sentence is "NoteDate:yyyy-mm-dd", added by Sunghwan Dec-2015
        ...
        if (senIter.hasNext()) {
            ...
            String date = "";
            if (senStr.startsWith("NoteDate:")) {
                dctAvailable = true;
                date = senStr.substring(9).replaceAll("-", ""); //yyyymmdd
                dctCentury = Integer.parseInt(date.substring(0, 2));
                dctYear = Integer.parseInt(date.substring(0, 4));
                dctDecade = Integer.parseInt(date.substring(2, 3));
                dctMonth = Integer.parseInt(date.substring(4, 6));
                dctDay = Integer.parseInt(date.substring(6, 8));
                dctQuarter = "Q"
...
//			System.out.println(date+"|"+dctCentury+"|"+dctYear+"|"+dctDecade+"|"+dctMonth+"|"+dctDay
//					+"|"+dctQuarter+"|"+dctHalf+"|"+dctSeason+"|"+dctWeekday+"|"+dctWeek);
        }
        //---
        // ////////////////////////////////////////////
        ...
        if (dctIter.hasNext()) {
            //System.out.println("has dctIter");
            dctAvailable = true;
                    ...
                    for (Object mr : Toolbox.findMatches(
                        ...
                        if (unit.equals("decade")) {
                            ...
                            if (dctAvailable
                                ...
                                String[] parts = valueNew.split("-");
                                if (op.equals("PLUS")) { //changed parts[3] to op, Sunghwan (10/19/2015)
...
//									if (parts[3].equals("PLUS")) {
                                //TODO changed by Sunghwan (2015-10-14), check if this is correct
                                if (op.equals("PLUS")) {
...
//									if (parts[3].equals("PLUS"))
    ...
    public boolean findTimexes(String timexType,
        ...
        for (Iterator<Pattern> i = Toolbox.sortByValue(hmPattern).iterator(); i
            ...
            //System.out.println("rule="+hmPattern.get(p));
                    ...
                    if (!(norm.getFromHmAllNormalization(((MatchResult) mr).group(1))
                        ...
                        Logger
                                .printDetail("Maybe problem with part to replace? "
                                        + partToReplace);
                        //---
                        System.out.println("Maybe problem with normalization of the resource: "
                                + ((MatchResult) mr).group(1));
                        System.out.println("Maybe problem with part to replace? "
                                + partToReplace);
                        //---
                    }
                    //System.out.println(((MatchResult) mr).group()+" "+partToReplace+((MatchResult) mr).group(1));
//					System.out.println(partToReplace);
//					System.out.println(norm.getFromHmAllNormalization(((MatchResult) mr).group(1)).get(partToReplace));
            ...
            for (Object mr : Toolbox.findMatches(paSum, tonormalize)) {
                //System.out.print("Integer.parseInt:"+((MatchResult) mr).group(1)+"|"+((MatchResult) mr).group(2));
